...
cte_deseq_obj <- readRDS("/restricted/projectnb/cteseq/jrose/cte_corr_work/cte_deseq_output_for_corr.rds")
...
#saveRDS(gsea_synGO, file = "cte_corr_gsea_synGO.rds")
#saveRDS(gsea_cp_c2, file = "cte_corr_gsea_cp_c2.rds")

```
all pathways
```{r}
gsea_synGO <- readRDS("cte_corr_gsea_synGO.rds")
gsea_cp_c2 <- readRDS("cte_corr_gsea_cp_c2.rds")
...
subsetting significant pathways
```{r}
gsea_synGO <- readRDS("cte_corr_gsea_synGO.rds")
gsea_cp_c2 <- readRDS("cte_corr_gsea_cp_c2.rds")
...
sig_gsea_cp_c2 <- lapply (
  names(gsea_cp_c2),
  function(i) {
    sig_sub_c2_cp <- subset(gsea_cp_c2[[i]], subset = gsea_cp_c2[[i]]$padj <= 0.05)
    return(sig_sub_c2_cp)
  }
)
names(sig_gsea_cp_c2) <- deseq_obj_names

sig_gsea_synGO <- lapply (
  names(gsea_synGO),
  function(i) {
    sig_sub_synGO <- subset(gsea_synGO[[i]], subset = gsea_synGO[[i]]$padj <= 0.05)
    return(sig_sub_synGO)
  }
)
names(sig_gsea_synGO) <- deseq_obj_names


for (i in names(sig_gsea_cp_c2)) {
  pathway_list_c2_cp <- append(pathway_list_c2_cp, sig_gsea_cp_c2[[i]]$pathway)
}

for (i in names(sig_gsea_synGO)) {
  pathway_list_synGO <- append(pathway_list_synGO, sig_gsea_synGO[[i]]$pathway)
}
...
#pdf("/restricted/projectnb/cteseq/jrose/cte_corr_work/nes_corr.pdf")
...
nes_clust_order_meta_1 <- subset(merged_meta_cor, select = c("CDStot", "tbri", "PathAD", "AT8_total", "tmi", "tgec", "DementiaHx", "faqtot", "apoe_de", "disdur", "aestot", "npftdtau", "CTE", "maxaggsum", "nppath", "TMEM106B_dom", "TMEM106B_invrec", "footyrs", "totyrs", "Group_de", "ParknismHx", "PathFTD", "nptdpe", "nptdpb", "nptdpc", "npold", "chii_f", "chii_g", "csparCTE", "cogdecage", "mincogage", "suicide", "AFE", "agecogsx", "sleepact", "bistot", "GDStot"))

nes_clust_order_meta <- subset(merged_meta_cor, select =c("aestot", "disdur", "CA1ratio", "CA4ratio", "subratio", "CA23ratio", "tbri", "CDStot", "npneur", "npdiff", "PathAD", "npadnc", "AT8_total", "npbraak", "npamy", "tmi", "tgec", "DementiaHx", "faqtot", "nphemo", "PathFTD", "Group_de", "nptdpe", "CTEStage", "ParknismHx", "nptdpb", "nptdpc", "npwmr", "npftdtau", "CTE", "maxaggsum", "TMEM106B_dom", "nppath", "footyrs", "totyrs", "npold", "TMEM106B_invrec", "chii_f", "chii_g", "csparCTE", "AFE", "agecogsx", "chii_r", "cogdecage", "mincogage", "sleepact", "npavas", "nparter", "bistot", "GDStot"))
...
#pdf("/restricted/projectnb/cteseq/jrose/cte_corr_work/meta_corr.pdf")
...
```{r}
par(mfrow = c(1,2))
corrplot(cp_c2_nes_corr_fun, order = 'hclust', tl.cex = 0.4, number.cex = 0.5)
corrplot(cor_meta_nes_order, tl.cex = 0.4, number.cex = 0.5)
