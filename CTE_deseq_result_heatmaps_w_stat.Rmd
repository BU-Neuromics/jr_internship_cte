...
all_results <- lapply(
  ...
  function(result_name) {
    ...
    results_c <- results_c[order(results_c$stat), ]
    l2fc_genes_test <- data.frame(ENSGID = rownames(results_c), result_name = results_c$stat)
...
for (i in filtered_names) {
  palette_length <- 50
  colors <- colorRampPalette(c("yellow", "white", "blue"))(palette_length)
  hm_breaks <- c(seq(min(heatmap_list[[i]]), 0, length.out=ceiling(palette_ength/2) + 1), 
...
for (i in filtered_names) {
  ...
  colors[50] <- "green"
  pheatmap(heatmap_list_10[[i]], cluster_rows = TRUE, cluster_cols = TRUE, color = colors, main = paste("Heatmap of significant (0.10) genes from ", i))
}
```
...
for (i in filtered_names) {
  colors <- colorRampPalette(c("blue", "white", "red"))(100)
  colors[50] <- "green"
  pheatmap(heatmap_list_01[[i]], cluster_rows = TRUE, cluster_cols = TRUE, color = colors, main = paste("Heatmap of significant (0.01) genes from ", i))
}
```
