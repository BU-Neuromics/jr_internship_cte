...
#!!!Ensure categorical data are factors before lapply deseq
...
run_deseq <- mclapply(
  col_combined,
  function(col_name) {
    function_meta <- merged_meta[!is.na(merged_meta$Year) & merged_meta$Year == 2021, ]
    function_meta <- function_meta[!is.na(function_meta[[col_name]]), ]
    function_counts <- rounded[, names(rounded) %in% function_meta$Core_ID]
    design_form <- formula(paste("~ agedeath + RIN + AT8_total + Batch +", col_name))
...
saveRDS(run_deseq, "/restricted/projectnb/cteseq/jrose/cte_deseq_output_ARTB_2021_model.rds")
