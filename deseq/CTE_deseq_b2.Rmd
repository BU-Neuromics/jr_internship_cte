...
```
DESeq2 high/low group and AT8_total (paper used rin, agedeath and batch in addition to covariate)
...
#
design_form <- formula(paste("~ agedeath + RIN + AT8_total"))
...
```{r}
test_meta <- merged_meta[!is.na(merged_meta$Group_de), ]
test_meta_low <- test_meta[!is.na(test_meta$Group_de) & test_meta$Group_de == "low", ]
test_meta_high <- test_meta[!is.na(test_meta$Group_de) & test_meta$Group_de == "high", ]
