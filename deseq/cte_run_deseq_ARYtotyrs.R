...
merged_meta <- subset(merged_meta, !is.na(merged_meta$totyrs))
...
run_deseq <- mclapply(
  ...
  function(i) {
    ...
    design_form <- formula(paste("~ agedeath + RIN + Year + totyrs +", i))
...
saveRDS(run_deseq, "/restricted/projectnb/cteseq/jrose/cte_corr_work/cte_deseq_output_for_corr_totyrs.rds")
