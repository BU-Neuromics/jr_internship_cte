...
col_categorical <- c("DementiaHx", "npold", "nppath", "npftdtau", "PathAD", "PathFTD", "CTE", "suicide", "apoe_de", "TMEM106B_dom", "TMEM106B_invrec", "ParknismHx", "nptdpb", "nptdpc", "nptdpe", "csparCTE", "sleepact")
...
run_deseq_high <- mclapply(
  col_categorical,
  function(col_name) {
    function_meta <- merged_meta[!is.na(merged_meta[[col_name]]), ]
    function_meta <- merged_meta[!is.na(merged_meta$Group_de) & merged_meta$Group_de == "high", ]
...
names(run_deseq_high) <- col_categorical
saveRDS(run_deseq_high, "/restricted/projectnb/cteseq/jrose/cte_deseq_output_cat_binary_high.rds")
