...
ltohf_genes <- rowSums(filtered_zeros == 0) < 150
filtered_counts <- filtered_zeros[ltohf_genes,]
...
col_continuous <- c("footyrs", "AFE", "totyrs", "npavas", "npwmr", "npbraak", "npneur", "npadnc", "npdiff", "npamy", "nparter", "agedeath", "agecogsx", "cogdecage", "mincogage", "AT8sulcus", "AT8sulcusLog", "disdur", "AT8crest", "DLFC.WS.AT8..cell.mm2", "AT8crestLog")

col_categorical <- c("cod", "DementiaHx", "npold", "npoldd", "nppath", "nplbod", "npftdtau", "PathAD", "PathLBD", "PathFTD", "PathMND", "CTE", "CTEStage", "micsuptemp", "suicide", "Group_de", "apoe_de", "TMEM106B_dom", "TMEM106B_invrec")

col_exclude <- c("SampleName", "RIN", "Core_ID", "race", "nphemo", "npold1", "npold2", "npold3", "npold4", "npoldd1", "npoldd2", "npoldd3", "npoldd4", "nppath6", "nppick", "npftdt2", "npcort", "npprog", "npftdt5", "npftdt8", "npftdt9", "npftdt10", "npftdtdp", "micdorfront", "micinfpar", "micalc", "locratio", "subratio", "CA1ratio", "CA23ratio", "CA4ratio", "sport", "Group", "rs1990622", "rs3173615", "apoe", "PathPrion", "npoftd")
...
names(run_deseq) <- col_continuous
saveRDS(run_deseq, "/restricted/projectnb/cteseq/jrose/cte_deseq_output_cont.rds")
...
run_deseq <- lapply(
  ...
  function(col_name) {
    function_meta <- merged_meta[!is.na(merged_meta[[col_name]]), ]
    function_counts <- rounded[, names(rounded) %in% function_meta$Core_ID]
    design_form <- formula(paste("~ agedeath + RIN +", as.factor(col_name)))
    dds <- DESeqDataSetFromMatrix(countData = function_counts,
                                  colData = function_meta,
                                  design = design_form)
    dds <- DESeq(dds)
...
saveRDS(run_deseq, "/restricted/projectnb/cteseq/jrose/cte_deseq_output_cat.rds")
